    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Chỉnh sửa màn hình {{ $data->screenId }}</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                ...
                <div class="row justify-content-md-center">
                    <div class="col-sm-12">
                        <div class="card card-info">
                            ...
                            <div class="card-body">
                                <form action="{{ route('screen.update', [$data->id]) }}" method="POST">
                                @csrf
                                    <div class="form-group">
                                        <label for="screenId">ID màn hình</label>
                                        <input type="text" id="screenId" value="{{ $data->screenId }}" name="screenId" class="form-control"
                                                  placeholder="Id của màn hình" />
                                    </div>

                                    <div class="form-group">
                                        <label for="screenName">Tên màn hình</label>
                                        <input type="text" id="screenName" value="{{ $data->screenName }}" name="screenName" class="form-control"
                                               placeholder="Tên màn hình" />
                                    </div>

                                    <div class="form-group">
                                        <label for="typeLog">Loại Log</label>
                                        <input type="text" id="typeLog" value="{{ $data->typeLog }}" name="typeLog" class="form-control"
                                               placeholder="Loại Log" />
                                    </div>

                                    <div class="form-group">
                                        <label for="api_url">URL</label>
                                        <input type="text" id="api_url" value="{{ $data->api_url }}" name="api_url" class="form-control"
                                               placeholder="URL" />
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea type="text" id="example_code" name="example_code" class="form-control"
                                               placeholder="Code mẫu">{{ $data->example_code }}</textarea>
                                    </div>

                                    <div class="form-group">
                                        <div class="form-check">
                                            <input id="status" name="status" class="form-check-input" type="checkbox" value="{{ $data->status }}" {{ ($data->status) ? 'checked' : '' }}>
                                            <label>Status</label>
                                        </div>
                                    </div>
