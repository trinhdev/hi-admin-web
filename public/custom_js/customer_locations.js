function getDetail(_this) {
    ...
    window.location.href = `/customer_locations/edit/` + infoRow.getAttribute('data-customer_location_id');
}

function deleteDetail(_this) {
    let row = _this.closest('tr');
    let infoRow = row.querySelector('.infoRow');
    window.location.href = `/customer_locations/destroy/` + infoRow.getAttribute('data-customer_location_id');
}
